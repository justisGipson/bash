...
# There is an implied loop and AWK automatically reads and parses each record of each file supplied
# Each record is split by the FS delimiter, which defaults to white-space (multiple spaces, tabs count as one)
# You can assign FS either on the command line (-F C) or in your BEGIN pattern

# One of the special patterns is BEGIN. The BEGIN pattern is treu BEFORE any of the files are read
# The END pattern is true after adn End-of_File form the last file (or standard-in if no files specified)
# There is also an output field separator (OFS) that you can assign, which defaults to a single space

BEGIN {
  # BEGIN will run at the beginning of the program. IT's where you put all the preliminary set-up code, before
  # you process any text files. If you have no text files, then think of BEGIN as the main entry point
  ...
  # you can also initialize an array with the build-in function split()
  n = split("foo:bar:baz", arr, ":");

  # You can also have assoociative arrays, with some limitations...
  multidim[0,0] = "foo";
  multidim[0,1] = "bar";
  multidim[1,0] = "baz";
  multidim[1,1] = "boo";

  # You can also test for array membership
  if ("foo" in assoc)
    print assoc[key];
...
# Here's how to define functions
function arithmetic_functions(a, b, c,      d) {
  # probably the most annoying part of AWk is that there are no local variables
  # Everything is global. For short scripts, this is fine, even useful, but for longer scripts
  # this can be a problem

  # There is a work-around. Function arguments are local to the function, and AWK allows you to
  # define more function arguments than it needs. So just stick local variable in the function declaration
  # like above. As a convention, put some extra whitespace to distiguish between actual function parameters
  # and local variables. In the example a, b, c are actual parameters, while d is a local variable

  # Now to demostrate arithmetic functions
  ...
  # Random Numbers
  srand(); # Supply a seed as an argument. By default, it uses time of day
...
function string_functions(    localvar, arr) {
  # AWk, being a string processing language, has several string-related functions
  # many of which rely heavily on regular expressions

  # Search and Replace, first instance (sub) or all instances (gsub)
  # Both return number of matches replaced
  localvar = "foooooobar";
  sub("fo+", "Meet me at the ", localvar); # localvar => "Meet me at the bar"
  gsub("e+", ".", localvar); # localvar => "m..t m. at the. bar"

  # Search for a string that matches a regular expression index() does the same thing, but doesn't allow
